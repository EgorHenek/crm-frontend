  ...
  <v-layout align-center justify-center row wrap>
      ...
      <v-list two-line>
        ...
        >
          <v-list-tile-content>
            ...
            <v-list-tile-sub-title v-html="item.attributes.description_html"></v-list-tile-sub-title>
          </v-list-tile-content>

          <v-list-tile-action>
            <v-list-tile-action-text v-if="item.attributes.published_at" :class="dateTimeColor(item.attributes.published_at)">{{ item.attributes.published_at| dateFormat('DD.MM.YYYY HH:mm')}}</v-list-tile-action-text>
            <v-icon @click.stop="delete_news(item.id)">fas fa-trash</v-icon>
          </v-list-tile-action>
        </v-list-tile>
      </v-list>
    </v-flex>

    <v-dialog v-model="edit_dialog" fullscreen hide-overlay v-if="edit_dialog">
      <v-card>
        ...
        <v-card-text>
          <news-form :news="editable_news.attributes" :errors2="edit_errors"/>
        </v-card-text>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialog" fullscreen hide-overlay>
      <v-card>
        ...
        <v-card-text>
          <news-form :news="new_news" :errors2="add_errors"/>
        </v-card-text>
      </v-card>
    </v-dialog>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add () {
        let publishedAt = this.new_news.not_published ? null : this.published_at(this.new_news.date_published_at, this.new_news.time_published_at)
